-- 20240116000007_extract_v3_nodefaults.sql
-- Creates rpc_extract_v3 with NO defaults to ensure strict signature matching.

CREATE OR REPLACE FUNCTION public.rpc_extract_v3(p_user_id UUID, p_seismic_grade TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION public.rpc_extract_v3(UUID, TEXT) TO service_role, authenticated, anon;
