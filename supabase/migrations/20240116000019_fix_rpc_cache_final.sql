-- 20240116000019_fix_rpc_cache_final.sql
-- Force cache reload and ensure permissions for rpc_extract_json

-- Re-declare to ensure PostgREST sees the latest signature
CREATE OR REPLACE FUNCTION public.rpc_extract_json(payload JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION public.rpc_extract_json(JSONB) TO service_role, authenticated, anon;

-- Force PostgREST notification
