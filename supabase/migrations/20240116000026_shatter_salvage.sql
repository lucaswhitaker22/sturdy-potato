-- Shatter Salvage (The Reaction Beat) Implementation
-- 1. Add cursed_fragment_balance to profiles
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS cursed_fragment_balance BIGINT DEFAULT 0 CHECK (cursed_fragment_balance >= 0);

-- 2. Add salvage columns to lab_state
ALTER TABLE public.lab_state
ADD COLUMN IF NOT EXISTS salvage_token UUID DEFAULT NULL,
ADD COLUMN IF NOT EXISTS salvage_expires_at TIMESTAMPTZ DEFAULT NULL,
ADD COLUMN IF NOT EXISTS pending_salvage_dust INT DEFAULT 0,
ADD COLUMN IF NOT EXISTS pending_salvage_fragment BOOLEAN DEFAULT FALSE;

-- 3. Update rpc_sift to support Shatter Salvage
...
BEGIN
    ...
    ELSE
        ...
        ELSE
            -- STANDARD FAIL - Shatter Salvage Opportunity
            ...
            ELSIF v_stage >= 3 THEN
                -- 10% base chance for Cursed Fragment on Stage 3+ shatter
                IF random() < 0.10 THEN
            ...
            UPDATE public.lab_state SET 
                ...
                salvage_expires_at = NOW() + INTERVAL '1.15 seconds', -- 1s + small grace
...
-- 4. Create rpc_shatter_salvage
CREATE OR REPLACE FUNCTION public.rpc_shatter_salvage(
    p_user_id UUID DEFAULT NULL,
...
BEGIN
    ...
    IF v_lab IS NULL OR v_lab.salvage_token IS NULL OR v_lab.salvage_token != p_token THEN
        RETURN jsonb_build_object('success', false, 'error', 'Invalid or expired salvage token');
    END IF;
    ...
    IF v_lab.salvage_expires_at < NOW() THEN
        ...
        UPDATE public.lab_state SET 
            ...
            pending_salvage_fragment = FALSE 
        WHERE user_id = v_user_id;
        RETURN jsonb_build_object('success', false, 'outcome', 'SALVAGE_MISSED');
    END IF;

    -- Valid Salvage
    v_dust_gain := v_lab.pending_salvage_dust;
    IF v_lab.pending_salvage_fragment THEN
        v_fragment_gain := 1;
    END IF;

    UPDATE public.profiles SET 
        fine_dust_balance = fine_dust_balance + v_dust_gain,
        cursed_fragment_balance = cursed_fragment_balance + v_fragment_gain
    WHERE id = v_user_id;
    ...
    RETURN jsonb_build_object(
        'success', true, 
        'outcome', 'SALVAGED',
        'dust_gain', v_dust_gain,
        'fragment_gain', v_fragment_gain
    );
END;
$$;

-- 5. Permissions
GRANT EXECUTE ON FUNCTION public.rpc_shatter_salvage(UUID, UUID) TO service_role, authenticated, anon;
