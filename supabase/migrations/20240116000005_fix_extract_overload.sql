-- 20240116000005_fix_extract_overload.sql
-- Drops old signature and re-creates rpc_extract to ensure schema cache is clean.

-- 1. Drop the old function explicitly to remove ambiguity
DROP FUNCTION IF EXISTS public.rpc_extract(UUID);

-- 2. Drop the new one too just to be sure we are clean
DROP FUNCTION IF EXISTS public.rpc_extract(UUID, TEXT);
