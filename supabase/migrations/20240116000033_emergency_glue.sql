-- 20240116000033_emergency_glue.sql
-- 1. Add failed_stage to lab_state
ALTER TABLE public.lab_state
ADD COLUMN IF NOT EXISTS failed_stage INT DEFAULT 0;

-- 2. Update rpc_sift_v2 to track failed_stage and allow tokens on critical fail
...
BEGIN
    ...
    ELSE
        ...
        -- Generate token (first 1s window)
        v_salvage_token := gen_random_uuid();

        IF v_is_critical THEN
            -- UPDATE: On Shatter, we still offer a 1s window for Glue if Lv 60+
            UPDATE public.lab_state SET 
                is_active = FALSE, 
                current_stage = 0, 
                failed_stage = v_stage, -- Record for Glue
                last_action_at = NOW(),
...
-- 3. Implement rpc_use_emergency_glue
CREATE OR REPLACE FUNCTION public.rpc_use_emergency_glue(
    p_user_id UUID DEFAULT NULL,
...
BEGIN
    ...
    IF v_restoration_level < 60 THEN
        RETURN jsonb_build_object('success', false, 'error', 'Restoration Level 60 required');
    END IF;

    IF COALESCE(v_profile.cursed_fragment_balance, 0) < 1 THEN
        RETURN jsonb_build_object('success', false, 'error', 'Insufficient Cursed Fragments');
    END IF;

    SELECT * INTO v_lab FROM public.lab_state WHERE user_id = v_user_id;

    IF v_lab IS NULL OR v_lab.salvage_token IS NULL OR v_lab.salvage_token != p_token THEN
        RETURN jsonb_build_object('success', false, 'error', 'Invalid or expired emergency window');
    END IF;

    IF v_lab.salvage_expires_at < NOW() THEN
        RETURN jsonb_build_object('success', false, 'error', 'Emergency window closed');
    END IF;

    -- DEDUCT COST
    UPDATE public.profiles SET cursed_fragment_balance = cursed_fragment_balance - 1 WHERE id = v_user_id;

    -- PERFORM CLAIM AT FAILED STAGE
    -- We temporarily Reactivate the lab at the failed stage to reuse rpc_claim
    UPDATE public.lab_state SET 
        is_active = TRUE, 
    ...
    -- Compensate for the tray_count decrement that happened in rpc_sift_v2
    UPDATE public.profiles SET tray_count = tray_count + 1 WHERE id = v_user_id;

    v_claim_result := public.rpc_claim(p_user_id);

    IF (v_claim_result->>'success')::BOOLEAN THEN
        RETURN jsonb_build_object(
            'success', true,
            'outcome', 'STABILIZED',
            'item', v_claim_result->'item',
            'new_fragment_balance', (SELECT cursed_fragment_balance FROM public.profiles WHERE id = v_user_id)
        );
    ELSE
        RETURN jsonb_build_object('success', false, 'error', 'Claim failed: ' || (v_claim_result->>'error'));
    END IF;
END;
$$;

GRANT EXECUTE ON FUNCTION public.rpc_use_emergency_glue(UUID, UUID) TO anon, authenticated, service_role;
